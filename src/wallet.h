...
public:
    ...
    //bitfrog
    bool fNetworkPaused;
    CShareWalletViewDB *psharewalletview;
    std::set<CTxDestination> setShareWalletaddr;
  ...
  //void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true) const;
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, bool fShareWallet = false) const;   //bitfrog
    ...
    //bitfrog
    bool IsShareWallet(const CTxIn &txin) const;

    //bitfrog
    bool IsShareWallet (const CTransaction& tx) const;

    //bitfrog
    bool IsShareWallet (const CTxOut& txout) const;

    //bitfrog
    void SetShareWallet (const CBitcoinAddress& addr);

    //bitfrog
    int64 GetShareWalletDebit(const CTxIn& txin) const;
    bool IsFromShareWallet(const CTransaction& tx) const
    {
        return (GetShareWalletDebit(tx) > 0);
    }

    //bitfrog
    /*
    void UpdateBloomFilterOnNewTxn(const CTransaction& tx, const uint256& hash);
    */
    ...
    //bitfrog
    CBloomFilter* GetCurrentBloomFilter ();

    //bitfrog
    void ClearShareWallet ();

    //bitfrog
    void ResyncAllBlocks();

    int64 GetShareWalletDebit(const CTransaction& tx) const
    {
        int64 nDebit = 0;
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            nDebit += GetShareWalletDebit(txin);
