...
uint64 nLocalServices = NODE_BLOOM;   //bitfrog
...
//bitfrog: send filterload to new peer
void CNode::PushFilter()
{
//    try
//    {
        if (fDebug) printf("send bloom filter to peer\n");
        CBloomFilter* currentfilter = pwalletMain->GetCurrentBloomFilter();
        //CBloomFilter filter = new CBloomFilter(&currentfilter);
        PushMessage("filterload", *currentfilter);
//    }
//    catch (std::exception& e) {
//        printf("Failed to send bloom filter to peer (exception)\n");
//        this->fDisconnect = true;
//    } catch (...) {
//        printf("Failed to send bloom filter to peer (...)\n");
//        this->fDisconnect = true;
//    }
}
