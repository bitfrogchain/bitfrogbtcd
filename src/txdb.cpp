...
//bitfrog
CShareWalletViewDB::CShareWalletViewDB(size_t nCacheSize, bool fMemory, bool fWipe) : db(GetDataDir() / "sharewallet", nCacheSize, fMemory, fWipe) {
}

//bitfrog
bool CShareWalletViewDB::Refresh () {
    CLevelDBBatch batch;

    std::vector<CKeyID> vKeyID;
    GetShareWallet(vKeyID);
    BOOST_FOREACH (const CKeyID& keyID, vKeyID) {
        batch.Erase(make_pair('a', keyID));
    }
...
//bitfrog
bool CShareWalletViewDB::SetShareWallet(const CKeyID &keyID) {
    CLevelDBBatch batch;
    batch.Write(make_pair('a', keyID), keyID);
    return db.WriteBatch(batch);
}

//bitfrog
bool CShareWalletViewDB::GetShareWallet(std::vector<CKeyID>& vWallets) {
...
/*
bool CBlockTreeDB::EraseAllBlockIndex()
{
    Erase(make_pair('b', blockindex.GetBlockHash()), true);
    return true;
}
*/
