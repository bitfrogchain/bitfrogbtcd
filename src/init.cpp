...
//    fReindex = GetBoolArg("-reindex");
    ...
    //bifrog: add share address on startup
    if (mapArgs.count("-shareaddr")) {
        BOOST_FOREACH(string strAddr, mapMultiArgs["-shareaddr"]) {

//string strAddr = "1FLv3gVJ8KAtDPa41LuD6NNXH5bj79xW4J";
//string strAddr = "1Q2TWHE3GMdB6BZKafqwxXtWAWgFt5Jvm3";
            CBitcoinAddress addr (strAddr);
            if (!addr.IsValid())
                printf("Invalid share wallet address: %s\n", strAddr.c_str());
            pwalletMain->SetShareWallet(addr.Get());
            pwalletMain->fNetworkPaused = false;    //allow sending message
        }
    }
